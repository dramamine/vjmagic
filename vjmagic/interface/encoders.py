...
def set_display_mode(mode, tolabels):
  ...
  if mode == 'BASIC':
    ...
    touched = [False] * 9
    update_display = update_display_touchy
  else:
...
def handle_push_turns(event):
  ...
  if data2 < 64:
    new_cc = min(127, values[encoder] + (SENSITIVITY * data2) )
  else:
    new_cc = max(0, values[encoder] - (SENSITIVITY * (128 - data2) ) )
...
def update_display_basic():
  ...
  val_strings = map(lambda x: str(x).center(8), values[:active_knobs])
  # annoying spacing
  outstr = ''
  ctr = 0
  ...
  outpututils.set_display_cells(1,
    labels + [''] * (8 - len(labels)) # pad to 8 with empty strings
  )
...
def update_display_touchy():
  val_cells = map(lambda x: 'x'*8 if x else ' '*8, touched[:active_knobs])
